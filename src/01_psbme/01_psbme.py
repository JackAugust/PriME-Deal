#!/usr/bin/env python3
"""
MJ18 (VFPPBA) Benchmark on SS512
=================================
Original code from "Privacy‑preserving bilateral access control"
Patched to run standalone (removed external deps, added stubs).
"""
import random
...
# ========== 补丁：替代原项目中无法导入的模块 ==========
# 原代码依赖以下文件：
#   from src.common.image import *   (未使用，可忽略)
#   from src.common.msp import *     (仅需空类)
#   from src.common.id import *      (提供 id_generator 等)
#   from src.common.parse import *   (提供 seq_func, output_func 等)
# 我们直接实现所需函数，保证逻辑与原文一致。

def seq_func():
    """每个配置重复次数，原文未明确，使用 3 次平均"""
    return 3

def output_func(base_name):
    """生成输出文件名"""
    return f"{base_name}_result.txt"

def id_generator(n):
    """生成随机的字母数字串作为身份"""
    return ''.join(random.choices(string.ascii_uppercase + string.digits, k=n))

def id_generate_func(n):
    """别名"""
    return id_generator(n)

def id_generator_digits(length):
    """生成随机数字串作为消息"""
    return ''.join(random.choices(string.digits, k=length))

# MSP 类在原代码中仅用于初始化，未实际调用其功能，故提供空壳
class MSP:
